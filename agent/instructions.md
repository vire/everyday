...
1. Call `read-memory` to load prior baselines and open suggestions. It returns `memories`: one entry per repo, each with `{ repo, content }`. Keep every `repo` — you will need it for step 8.
...
8. Call `write-memory` with `{ memories: [...] }` — one entry per repo, each pairing that repo's `repo` slug (from step 1) with its updated memory document containing:
