...
1. Call `read-memory` to load prior baselines and open suggestions. It returns `memories`: one entry per repo, each with `{ repo, ref, content }`. Keep every `ref` (paired with its `repo`) — you will need them for step 8.
...
8. Call `write-memory` with `{ memories: [...] }` — one entry per repo, each pairing that repo's `ref` from step 1 with its updated memory document containing:
